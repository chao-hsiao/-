...
def results():
    ...
    if connection:
        ...
        for row in rows:
            table_html += "<tr>"
            table_html += f"<td><form action='delete_item' method='post'><input type='hidden' id='delete_id' name='delete_id' value='{row['district_id']}'><input type='submit' value='delete' onclick='return confirm(\"delete?\")'></form></td>"
