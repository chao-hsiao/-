...
def upload():
    ...
    if connection:
        ...
        if columns != "":
            cursor.execute(f"INSERT INTO lvr_land_c ({columns}) VALUES ({values});")

        columns = ', '.join(upload_park.keys())
        values = "'" + "', '".join(upload_park.values()) + "'"
        if columns != "":
            cursor.execute(f"INSERT IGNORE INTO lvr_land_c_park ({columns}) VALUES ({values});")
...
def cont():
    if request.method == 'POST':
        ...
        if connection:
            ...
            cursor.execute(f"INSERT INTO lvr_land_c_build ({columns}) VALUES ({values});")
            columns = ', '.join(upload_land.keys())
            values = "'" + "', '".join(upload_land.values()) + "'"
            cursor.execute(f"INSERT INTO lvr_land_c_land ({columns}) VALUES ({values});")
            connection.commit()
            cursor.close()
            connection.close()
            return render_template('alert_msg.html',msg=True)
...
def delete_item():
    ...
    connection = get_db_connection()
    if connection:
        sql = f"DELETE FROM lvr_land_c WHERE district_id = {request.form['delete_id']}"
